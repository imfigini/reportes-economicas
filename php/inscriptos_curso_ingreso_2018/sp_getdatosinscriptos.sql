...
BEGIN
{
	...
	INSERT INTO rep_datos_inscriptos
					SELECT DISTINCT SED.nombre AS sede, 
						A.nro_inscripcion,
					...
					FROM 	sga_alumnos A 
						JOIN sga_personas B ON (A.nro_inscripcion = B.nro_inscripcion)
						JOIN sga_carrera_aspira S ON (A.nro_inscripcion = S.nro_inscripcion AND A.carrera = S.carrera)
						JOIN sga_sedes SED ON (SED.sede = S.sede)
						JOIN vw_datos_censales_actuales D ON (D.nro_inscripcion = A.nro_inscripcion)
