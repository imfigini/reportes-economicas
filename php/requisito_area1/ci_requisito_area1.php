...
class ci_requisito_area1 extends toba_ci
{
	protected $s__datos_filtro;
	protected $s__lista_materias;
	protected $s__materia_con_mas_correlativas;
	...
	function evt__cuadro__verificar($datos)
	{
		ei_arbol($this->s__lista_materias);
		$this->set_pantalla('pant_resultado');
		//TO DO
		//Calcular cual es la matteria con más cantidad de correlativas
		$this->s__materia_con_mas_correlativas = $this->s__lista_materias[0];
	}	
	...
	function get_materia_con_mas_correlativas()
	{
		ei_arbol($this->s__materia_con_mas_correlativas, 'entro en get_materia_con_mas_correlativas');
		
		//TO DO
		//RECUPERAR EL NOMBRE DE LA MATERIA A MOSTRAR
	//	if (isset($s__materia_con_mas_correlativas))
		{
			//$result = array('MATERIA_CON_MAS_CORREL' => $this->s__materia_con_mas_correlativas);
			//ei_arbol ($result, 'result'	);
			return $this->s__materia_con_mas_correlativas;
		}
		
		//TO DO
		//Agregar cuadro con los alumnos que tienen aprobado el final de dicha materia
	}
	
	function conf__formulario(Reportes_ei_formulario $form)
	{
		ei_arbol($form, 'form');
	}
