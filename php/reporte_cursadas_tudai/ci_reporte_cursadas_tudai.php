...
class ci_reporte_cursadas_tudai extends toba_ci
{
	function get_cursadas_tudai()
	{
		$sql = "SELECT 	legajo, 
	...
	function get_ultima_actualizacion()
	{
		...
		$sql = "SELECT MAX(fecha_ultima_actualizacion) AS fecha_ultima_actualizacion
					FROM rep_fecha_actualiz_tablas
						WHERE tabla = 'rep_tudai_cursadas'";
	...
	function evt__tudai_cursadas_procesar__procesar($datos)
	{
		$db = MisConsultas::getConexion();
		$sql = "EXECUTE PROCEDURE 'dba'.sp_rep_tudai_cursadas()";
